...
void common_preset::merge(const common_preset & other) {
    if (!other.backend.empty()) {
        backend = other.backend; // carry backend type
    }
...
common_presets common_preset_context::load_from_ini(const std::string & path, common_preset & global) const {
    ...
    for (auto section : ini_data) {
        ...
        for (const auto & [key, value] : section.second) {
            ...
            if (key == "backend") {
                // store backend type (e.g. "rkllm") for model_instance dispatch
                preset.backend = value;
                LOG_DBG("accepted option: backend = %s\n", value.c_str());
                continue;
            }
            if (key == "max-context-len") {
                // RKLLM-specific: map to standard --ctx-size option
                // "c" is the short-form key for --ctx-size in key_to_opt
                auto it_opt = key_to_opt.find("c");
                if (it_opt != key_to_opt.end()) {
                    preset.options[it_opt->second] = value;
                    LOG_DBG("mapped max-context-len=%s to ctx-size\n", value.c_str());
                }
                continue;
            }
