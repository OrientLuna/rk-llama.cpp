...
void server_models::add_model(server_model_meta && meta) {
    ...
    // carry backend type from preset
    meta.backend = meta.preset.backend;
...
void server_models::load_models() {
    ...
    // preload any models marked load-on-startup in their preset (independent of
    // --no-models-autoload, which only governs on-demand loading of requests)
...
void server_models::load(const std::string & name) {
    ...
    if (inst.meta.backend == "rkllm") {
        std::string model_path;
        inst.meta.preset.get_option("LLAMA_ARG_MODEL", model_path);
        if (model_path.empty()) {
            throw std::runtime_error("rkllm backend requires a model path");
        }

        int max_ctx = 2048;
        std::string ctx_str;
        if (inst.meta.preset.get_option("LLAMA_ARG_CONTEXT_SIZE", ctx_str)) {
            try { max_ctx = std::stoi(ctx_str); } catch (...) {}
        }

        // Vision encoder path (mmproj field, typically .rknn for RKLLM multimodal)
        ...
        if (old_instance.th.joinable()) old_instance.th.join();
        mapping[name] = std::move(inst);
        cv.notify_all();
        SRV_INF("RKLLM model instance loaded: %s\n", name.c_str());
        return;
    }
#endif
...
static bool router_validate_model(std::string & name, server_models & models, bool models_autoload, const std::string & default_model, std::unique_ptr<server_http_res> & res) {
    if (name.empty()) {
        // fall back to the configured default model, if any
...
void server_models_routes::init_routes() {
    ...
    this->proxy_get = [this](const server_http_req & req) {
        ...
        if (!router_validate_model(name, models, autoload, params.default_model, error_res)) {
            return error_res;
        }
