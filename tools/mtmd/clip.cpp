...
#ifdef GGML_USE_RKNPU2
    rknn_encoder * rknn_enc = nullptr;
    bool rknn_enc_enabled = false;  // override: MTMD_USE_RKNN=1
#endif
...
#ifdef GGML_USE_RKNPU2
    // Auto-detect RKNN encoder: look for .rknn file with same basename as mmproj
    if (ctx_vision && !ctx_vision->rknn_enc) {
        std::string rknn_path = std::string(fname);
        auto dot = rknn_path.rfind('.');
        if (dot != std::string::npos) {
            rknn_path = rknn_path.substr(0, dot) + ".rknn";
        } else {
            rknn_path += ".rknn";
        }
        if (std::filesystem::exists(rknn_path)) {
            int n_merge = clip_get_hparams(ctx_vision)->n_merge;
            int proj_dim = clip_n_mmproj_embd(ctx_vision);
            ctx_vision->rknn_enc = rknn_encoder_init(rknn_path.c_str(), n_merge, proj_dim);
            if (ctx_vision->rknn_enc) {
                LOG_INF("%s: RKNN encoder loaded from %s\n", __func__, rknn_path.c_str());
                const char * env = std::getenv("MTMD_USE_RKNN");
                ctx_vision->rknn_enc_enabled = (env == nullptr) || (strcmp(env, "0") != 0);
            }
        }
    }
#endif
...
#ifdef GGML_USE_RKNPU2
    // RKNN encoder fast path (skip during warmup when vec is nullptr)
    if (ctx->rknn_enc && ctx->rknn_enc_enabled && vec != nullptr) {
        // Get preprocessed pixels (already normalized to [0,1] in NCHW format)
        const auto & img = *imgs.entries[0];
        int n_tokens = rknn_encoder_run(ctx->rknn_enc, img.buf.data(), img.nx, img.ny, vec);
        if (n_tokens > 0) {
            return true;
        }
        // RKNN failed, fall through to GGML path
        LOG_WRN("%s: RKNN encoder failed, falling back to GGML\n", __func__);
    }
#endif
